...
interface CalcBtn { label: string; type: 'number' | 'operator' | 'equals' | 'clear' | 'special'; value: string }
...
function handleBtn(btn: CalcBtn) {
  switch (btn.type) {
    case 'clear':
      ...
      break
    case 'number':
      currentInput.value += btn.value; expression.value += btn.value
      break
    case 'operator':
    ...
    case 'equals':
      try {
        const evalExpr = expression.value.replace(/×/g, '*').replace(/÷/g, '/').replace(/−/g, '-')
...
<style scoped>
.basic-calc { display: flex; flex-direction: column; height: 100%; }
...
.calc-expression {
  font-size: 1rem; color: var(--app-text-secondary, var(--text-muted));
  font-family: var(--font-mono); min-height: 1.4em; word-break: break-all;
}
...
.calc-grid {
  ...
  gap: var(--sp-sm); padding: 0 var(--sp-md) var(--sp-md);
}

.calc-btn {
  height: 56px; border-radius: var(--radius-lg);
  font-size: 1.2rem; font-weight: 500; border: none; cursor: pointer;
  background: var(--app-surface, var(--bg-primary));
  color: var(--app-text, var(--text-primary));
  transition: all var(--ease-fast); font-family: var(--font-sans);
}
.calc-btn:hover { background: var(--app-surface-hover, var(--bg-hover)); transform: scale(1.02); }
.calc-btn:active { transform: scale(0.98); }

.calc-btn-op {
  background: var(--app-surface-hover, var(--bg-hover));
  color: var(--app-primary, var(--accent-primary)); font-size: 1.4rem;
}
.calc-btn-op:hover { background: var(--app-primary, var(--accent-primary)); color: white; }

.calc-btn-eq {
  background: var(--app-primary, var(--accent-primary));
  color: white; font-size: 1.4rem;
}
.calc-btn-eq:hover { background: var(--app-primary-dark, var(--accent-primary-dark)); box-shadow: var(--shadow-md); }

.calc-btn-clear { color: var(--app-error, var(--accent-error)); font-weight: 600; }
.calc-btn-special { color: var(--app-text-secondary, var(--text-secondary)); font-weight: 600; }
</style>
