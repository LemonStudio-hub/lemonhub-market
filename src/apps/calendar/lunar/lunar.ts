...
// Based on lunar calendar data 1900-2100
const lunarInfo = [
...
function leapDays(y: number) { if (!leapMonth(y)) return 0; return (lunarInfo[y - 1900] & 0x10000) ? 30 : 29 }
function monthDays(y: number, m: number) { return (lunarInfo[y - 1900] & (0x10000 >> m)) ? 30 : 29 }

export function solarToLunar(year: number, month: number, day: number): { year: number; month: number; day: number; gzYear: string; gzMonth: string; gzDay: string; animal: string; monthStr: string; dayStr: string } {
  const baseDate = new Date(1900, 0, 31)
  const objDate = new Date(year, month - 1, day)
  let offset = Math.floor((objDate.getTime() - baseDate.getTime()) / 86400000)

  let i = 1900; let temp = 0
  for (i = 1900; i < 2101 && offset > 0; i++) { temp = lYearDays(i); offset -= temp }
  if (offset < 0) { offset += temp; i-- }

  const lunarYear = i
  const leap = leapMonth(i)
  let isLeap = false; let lunarMonth = 1
  for (lunarMonth = 1; lunarMonth < 13 && offset > 0; lunarMonth++) {
    if (leap > 0 && lunarMonth === (leap + 1) && !isLeap) { --lunarMonth; isLeap = true; temp = leapDays(i) }
    else { temp = monthDays(i, lunarMonth) }
  ...
  if (offset < 0) { offset += temp; --lunarMonth; if (lunarMonth < 1) { lunarMonth = 12 } }

  const lunarDay = offset + 1

  // Ganzhi
  const gzYear = Gan[(lunarYear - 4) % 10] + Zhi[(lunarYear - 4) % 12]
  const gzMonth = Gan[((lunarYear - 4) % 10 * 12 + (lunarMonth - 1) * 2 + 12) % 10] + Zhi[((lunarMonth - 1) * 2 + 12) % 12]
  const dayCyl = offset + 40
  const gzDay = Gan[dayCyl % 10] + Zhi[dayCyl % 12]
